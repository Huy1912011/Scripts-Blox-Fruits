...
Setting:Seperator("「 Credits 」")
    Setting:Label("Made By Hoàng Gia Huy")
...
repeat wait() until game:IsLoaded() 
 ...
 Setting:Toggle("Walk on Water",true,function(value)
        _G.WalkWater = value
    end)
...
Main:Seperator("「 ESP 」")
    ...
    Main:Button("Teleport To Fruit",function()
...
Dungeon:Toggle("Kill Aura 2",_G.Kill_Aura2,function(value)
 _G.Kill_Aura2 = value  
end)

spawn(function()
    while wait() do
        if _G.Kill_Aura2 then
            for i,v in pairs(game.Workspace.Enemies:GetDescendants()) do
                if v:FindFirstChild("Humanoid") and v:FindFirstChild("HumanoidRootPart") and v.Humanoid.Health > 1 then
                    pcall(function()
                        repeat wait(.1)
                            ...
                            v.HumanoidRootPart.CanCollide = false
							sethiddenproperty(game.Players.LocalPlayer, "SimulationRadius", math.huge)
                        until not _G.Kill_Aura2  or not v.Parent or v.Humanoid.Health <= 1
                    end)
                end
            end
        end
